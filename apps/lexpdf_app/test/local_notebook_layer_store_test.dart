...
void main() {
  ...
  test(
    'assigns strokes to layers and reassigns them on layer deletion',
    () async {
      final database = LocalDatabase.inMemory();
      addTearDown(database.close);
      final inkStore = LocalInkStore(database);
  ...
  test(
    'rejects cross-page assignments and cleans deleted stroke mapping',
    () async {
      final database = LocalDatabase.inMemory();
      addTearDown(database.close);
      final inkStore = LocalInkStore(database);
  ...
  test(
    'many layers and assignments survive reorder and database reopen',
    () async {
      const layerCount = 40;
      ...
      for (var index = 1; index < layerCount; index++) {
        created.add(
          await layers.createLayer(page.id, name: 'Camada ${index + 1}'),
        );
      }
      ...
      for (var index = 0; index < strokeCount; index++) {
        expect(
          assignments['stress-stroke-$index'],
          created[index % layerCount].id,
        );
      }
