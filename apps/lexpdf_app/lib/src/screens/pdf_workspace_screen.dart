...
import 'androidx_pdf_reader_launcher.dart';
...
class _PdfWorkspaceScreenState extends State<PdfWorkspaceScreen>
  ...
  void _startActiveIndexing() {
    ...
    if (Platform.isAndroid) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'OCR/indexação do workspace permanece desativado no novo leitor '
            'AndroidX durante a validação do motor nativo.',
          ),
        ),
      );
      return;
  ...
  Widget _buildEditorForTab(_WorkspaceTab tab) {
    ...
    if (Platform.isAndroid) {
      return AndroidxPdfReaderLauncher(
        key: key,
        document: tab.document,
        initialPage: tab.initialPage,
  ...
  Widget _buildPdfEditorSurface() {
    if (Platform.isAndroid) {
      // Android delegates the document to a native AndroidX PDF Activity in
      // its own process. Flutter never mounts the legacy pdfrx/PDFium viewer.
