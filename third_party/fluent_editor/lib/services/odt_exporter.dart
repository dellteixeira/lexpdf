...
class OdtExporter {
  ...
  Future<Uint8List> build() async {
    ...
    final mimeFile = ArchiveFile.noCompress(
      'mimetype',
      mimeBytes.length,
      mimeBytes,
    );
  ...
  String _ensureTrackedChange(Fragment frag, {String? paragraphId}) {
    ...
    final author = _esc(
      matchingSug?['authorName'] as String? ?? document.authorName,
    );
    ...
    if (isAddition) {
      buf.write(
        '<text:insertion><office:change-info><dc:creator>$author</dc:creator>',
      );
      if (dateStr.isNotEmpty) buf.write('<dc:date>$dateStr</dc:date>');
      buf.write('</office:change-info></text:insertion>');
    } else {
      buf.write(
        '<text:deletion><office:change-info><dc:creator>$author</dc:creator>',
      );
      if (dateStr.isNotEmpty) buf.write('<dc:date>$dateStr</dc:date>');
