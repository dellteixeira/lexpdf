...
# ---------------------------------------------------------------------------
# Selection actions: manual flashcards, rich anchored notes, no Question/Explain
# ---------------------------------------------------------------------------
...
text = replace_between(
    ...
    manual_method,
    'replace AI study action with manual flashcard',
)
...
note_method = r'''  Future<void> _createNoteFromSelection(
    ...
    if (document != null &&
      ...
      if (fragments.isNotEmpty) {
        ...
        y = ((page.height - bounds.top) / page.height)
            .clamp(0.02, 0.98)
            .toDouble();
      }
    }
              ...
              child: Column(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: SingleChildScrollView(
                      child: SelectableText(selectedText),
                    ),
                  ),
                  ...
                  Wrap(
                    ...
                    children: [
                      ...
                      DropdownButton<String>(
                        ...
                        onChanged: (value) {
                          if (value != null) {
                            setDialogState(() => fontFamily = value);
                          }
                        },
                      ),
                      const SizedBox(width: 8),
                      for (final option in const <(String, IconData)>[
                        ...
                        IconButton(
                          tooltip: 'Alinhamento ${option.$1}',
                          isSelected: textAlign == option.$1,
                          onPressed: () =>
                              setDialogState(() => textAlign = option.$1),
                          icon: Icon(option.$2),
                        ),
                    ],
                  ),
...
'''
if '_ManualFlashcardDraft {' in text:
...
# ---------------------------------------------------------------------------
# Sticky note renderer/editor: smaller pin + richer editing, backward compatible
# ---------------------------------------------------------------------------
...
text = replace_once(
    ...
    """    this.underline = false,\n    this.fontSize = 16,\n  });\n""",
    """    this.underline = false,\n    this.fontSize = 16,\n    this.fontFamily = 'Roboto',\n    this.textAlign = 'left',\n  });\n""",
    'sticky content constructor',
)
text = replace_once(
    text,
    """  final bool underline;\n  final double fontSize;\n""",
    """  final bool underline;\n  final double fontSize;\n  final String fontFamily;\n  final String textAlign;\n""",
    'sticky content fields',
)
text = replace_once(
    text,
    """      'underline': underline,\n      'fontSize': fontSize,\n""",
    """      'underline': underline,\n      'fontSize': fontSize,\n      'fontFamily': fontFamily,\n      'textAlign': textAlign,\n""",
    'sticky encode format',
)
text = replace_once(
    text,
    ...
    'sticky decode format',
)
text = replace_once(text, '  static const _markerSize = 34.0;', '  static const _markerSize = 26.0;', 'smaller note pin')
old_icon = """  Widget _markerIcon({double opacity = 1}) => Opacity(\n        opacity: opacity,\n        child: const Material(\n          color: Colors.transparent,\n          child: Icon(\n            Icons.sticky_note_2,\n            color: Color(0xFFFFC107),\n            size: 30,\n          ),\n        ),\n      );\n"""
new_icon = """  Widget _markerIcon({double opacity = 1}) => Opacity(\n        opacity: opacity,\n        child: const Material(\n          color: Colors.transparent,\n          child: Icon(\n            Icons.push_pin_rounded,\n            color: Color(0xFFFFB300),\n            size: 20,\n          ),\n        ),\n      );\n"""
text = replace_once(text, old_icon, new_icon, 'pin marker icon')
                ...
                children: [
                  Wrap(
                    ...
                    children: [
                      ...
                      const SizedBox(width: 8),
                      DropdownButton<String>(
                        value: const ['Roboto', 'sans-serif', 'serif', 'monospace']
                                .contains(fontFamily)
                            ? fontFamily
                            : 'Roboto',
                        tooltip: 'Tipo de fonte',
...
# ---------------------------------------------------------------------------
# Workspace: force sticky overlay refresh when a selection-created note is saved
# ---------------------------------------------------------------------------
...
# Regression contract
# ---------------------------------------------------------------------------
test_path = ROOT / 'test/manual_flashcard_rich_note_contract_test.dart'
...
void main() {
  test('selection study flow is manual and omits question/explain actions', () {
    final source = File('lib/src/widgets/pdf_selection_action_menu.dart')
        .readAsStringSync();
  ...
  test('selection notes are anchored rich-text pins', () {
    final menu = File('lib/src/widgets/pdf_selection_action_menu.dart')
        .readAsStringSync();
    final overlay = File('lib/src/widgets/pdf_sticky_note_overlay.dart')
        .readAsStringSync();
